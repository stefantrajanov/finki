...
public class ConsecutiveNumbers {
    private static boolean preorderTraversal(BNode<Integer> root) {
        if (root == null) {
            return true;
        }
        ...
        stack.push(root);
        BNode<Integer> lastNode = new BNode<>(root.info - 2);

        while (!stack.isEmpty()) {
            BNode<Integer> currentNode = stack.pop();
            if (currentNode.info - 2 != lastNode.info){
                return false;
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // vasiot kod ovde
        if (preorderTraversal(tree.root)){
