...
class Evaluator {
    ...
    public static <T extends Comparable<T>> boolean evaluateExpression(T left, T right, String operator) {
        // Use EvaluatorBuilder to build an evaluator and then evaluate the expression
        IEvaluator<T> evaluator = EEvaluatorBuilder.build(operator);
        return evaluator.evaluate(left, right); // Placeholder
    }
}
